...
struct Sampler //采样器，把模型输出的logits转换成下一个token
{
    ...
    float topp; //核采样参数
    std::uint64_t rng_seed; //随机数生成器的种子
    std::mt19937 rng;

    Sampler()=default; //默认构造函数
    explicit Sampler(int vocab_size_,float temperature_,float topp_,std::uint64_t rng_seed_) : vocab_size(vocab_size_),probindex(vocab_size_),temperature(temperature_),topp(topp_),rng_seed(rng_seed_)//带参数的构造函数。explicit的作用是禁止隐式类型转换
    {
        std::seed_seq seed{
            static_cast<std::uint32_t>(rng_seed),
            static_cast<std::uint32_t>(rng_seed >> 32)
        };
        rng.seed(seed);
    } 
};
...
void run(const InferenceOptions& options)
{
    ...
    {
        throw std::invalid_argument("tokenizer path must not be empty");
    }
    if(options.steps<0)
    {
    ...
    {
        throw std::invalid_argument("temperature must be a finite non-negative number");
    }
    if(!std::isfinite(options.top_p)||options.top_p<0.0f||options.top_p>1.0f)
    {
        throw std::invalid_argument("top-p must be a finite number between 0 and 1");
    }
    ...
    build_transformer(transformer,options.model_path);

    const int steps=options.steps==0?transformer.config.seq_len:options.steps;
    if(steps>transformer.config.seq_len)
    {
        throw std::invalid_argument("steps exceeds the model sequence length");
    }

    Tokenizer tokenizer;
    build_tokenizer(tokenizer,options.tokenizer_path,transformer.config.vocab_size);
    
    Sampler sampler(transformer.config.vocab_size,options.temperature,options.top_p,options.seed);

    generate(transformer,tokenizer,sampler,options.prompt,steps);
}
}
